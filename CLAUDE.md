...
- **`src/hooks/`** — `useGameClient` (wires the client into React state), `usePwaUpdate` (service-worker update prompt), `useWalkMode` (Walk Mode: DeviceMotion → `game/pedometer` step detection → clicks; an alternative input alongside the GROW button, throttled identically).
