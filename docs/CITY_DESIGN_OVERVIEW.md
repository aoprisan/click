...
- **Construction & upgrades — instant cash purchases** *(prototype: this pivoted from the original design).* Pay from the city's cash and the building (or +1 upgrade level) is operational immediately; its workers count toward population that instant. *(The original design gated construction behind activity units — "money is the gate, clicking is the work" — but the `web/` prototype made it instant so clicks are reserved purely for production; see [`CORE_LOOP.md`](CORE_LOOP.md) §1a.)* Upgrades raise the worker count and the output per batch.
- **Production runs on the city's own stock.** A building's batch consumes its inputs from inventory and **stalls if any input is missing** — nothing is auto-bought. Feed it by producing upstream, buying from the global market by hand, or trading. **Clicks drive production**: each click banks work toward the next batch (scaled by happiness), and a building missing an input can't be worked at all. Per-batch input/output **amounts** are a hand-authored overlay (`src/game/recipes.ts`) over the CSV's ingredient lists.
