...
- **`src/game/`** — pure, unit-tested logic: `catalog` (buildings/prices from the
  CSVs), `recipes` (hand-authored input/output amounts overlaid on the generated
  recipes), `economy` (construction + production), `population`, `happiness`,
...
- `game/catalog.ts` — `workersForTier` (population per building level),
  `buildCost` / `upgradeCost` / `workPerBatch` curves.
- `game/recipes.ts` — hand-authored per-building input/output **amounts** (the
  supply-chain weights that gate production).
